...
private:
    ...
    array<int, 2> emphasize; // 重視する科目

public:
    School(double std_score = 50.0, array<int, 2> emp = {0, 1}) : nodes(CAPACITY, Node()), standard_score(std_score), emphasize(emp) {}
    ...
    const array<int,2>& get_emphasize() const {
        return emphasize;
    }
};
...
int main() {
    ...
    for (int sch = 0; sch < M; sch++) { // 偏差値が高い学校ほど idx が小さい
        schools.emplace_back(scores[sch], EMP[sch]);
    }
    ...
    for (int sch = 0; sch < M; sch++) {
         ...
         * ➂
         */
        auto [sub1, sub2] = schools[sch].get_emphasize(); // 重視する科目
        auto cmp3 = [&](int a, int b) {
            int suma = 0, sumb = 0;
            // 1 つ目の重視する科目
            for (int sub = 0; sub < SUBJECTS; sub++) {
                int w = (sub == sub1) ? 2 : 1;
                suma += w * students[a].get_point(sub);
                sumb += w * students[b].get_point(sub);
            }
            if (suma != sumb) {
                return suma > sumb;
            }
            return students[a].get_ID() < students[b].get_ID();
        };
        ...
        // 重視する科目➁の割合
        int rat2 = 20;
        int A = CAPACITY * rat1 / 100;
        int B = CAPACITY * rat2 / 100;
        int C = CAPACITY - A - B;
        schools[sch].divide({ {cmp3, A}, {cmp4, B}, {cmp1, C} });
    }
     ...
     */
    for (int s = 0; s < N; s++) {
        const auto& pts = students[s].get_points();
        array<int, SUBJECTS> ord;
        iota(ord.begin(), ord.end(), 0);
        sort(ord.begin(), ord.end(), [&](int a, int b){ return pts[a] > pts[b]; });

        int top1 = ord[0], top2 = ord[1];

        vector<tuple<int, int, int>> tmp;
        for (int sch = 0; sch < M; sch++) {
            const auto& emp = schools[sch].get_emphasize();
            // 成績の良かった上位 2 つの科目のうち、
            // 両科目を重視してくれる学校を最優先
            // 片方の科目だけを重視してくれる優先を次点
            int match_cnt = (emp[0] == top1 || emp[0] == top2) +
                            (emp[1] == top1 || emp[1] == top2);
            int score = 0;
            for (int sub = 0; sub < SUBJECTS; sub++) {
                int w = (sub == emp[0] || sub == emp[1]) ? 2 : 1;
                score += w * pts[sub];
            }
            tmp.emplace_back(-match_cnt, -score, sch);
        }
        sort(tmp.begin(), tmp.end());

        for (int i = 0; i < M; i++) {
            school_prefs[s][i] = get<2>(tmp[i]);
        }
    }
